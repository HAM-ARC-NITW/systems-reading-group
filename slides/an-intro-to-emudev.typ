...
In hardware emulation, the target system's circuitry is physically reconstructed, either built from scratch or implemented by reconfiguring the host hardware (using an FPGA) so the host hardware literally behaves like the original gates. Some of the common projects in terms of hardware emulation are building a #link("https://eli.lipsitz.net/posts/fpga-gameboy-emulator/")[FPGA-based Game Boy emulator], #link("https://tubetime.us/index.php/2016/05/15/placeholder/")[Transistor-scale replica of the MOS 6502 CPU]
...
#place(center, dx: 180pt, dy: 20pt, figure(
...
#place(center, dx: -200pt, dy: 20pt, figure(
  image("_assets/ps1-web-emulator.png", height: 270pt),
  caption: [#link("https://github.com/maxpoletaev/nupsx")[nuPSX]: Web-based PlayStation 1 emulator]
))
...
- *Compatibility layers*: No OS in the loop. The layer intercepts the API calls a program makes and reimplements them with host APIs. #link("https://www.winehq.org/")[Wine] and Proton do this for Windows software on Linux. Much lighter than a VM, but only covers the APIs the layer implements.

== Scope of this presentation

After getting an overview of the different ways emulators are built and also the different ways to "fake", I would now mainly talk about software emulation in deeper using an interpreter-based #link("https://github.com/0xmukesh/tiny.nes")[NES emulator] as a reference. Initially, I would cover the basics of NES's hardware architecture and later I will map those topics with equivalent code sections to get a better idea on how software emulation is done.
